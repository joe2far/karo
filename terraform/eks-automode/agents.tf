# -----------------------------------------------------------------------------
# Example Agent Team — Bedrock + Claude Code Harness (EKS Auto Mode)
# -----------------------------------------------------------------------------
...
# ModelConfig: AWS Bedrock
resource "kubernetes_manifest" "bedrock_model_config" {
  count = var.enable_example_agents ? 1 : 0
  manifest = {
    ...
    spec = {
      ...
      name     = var.bedrock_model_id
      bedrock = {
        region      = var.region
        irsaRoleArn = aws_iam_role.karo_agent_bedrock.arn
      }
      parameters = {
        maxTokens   = 8192
        temperature = 0.3
...
# SandboxClass: secure execution environment for EKS
    ...
    spec = {
      ...
      networkPolicy = {
        ...
        allowedDomains = [
          ...
          "go.dev",
          "proxy.golang.org",
          "bedrock-runtime.${var.region}.amazonaws.com",
          "sts.amazonaws.com",
        ]
        allowedCIDRs = []
      }
...
resource "kubernetes_manifest" "planner_agent" {
  ...
  depends_on = [helm_release.karo, kubernetes_manifest.bedrock_model_config]
}
...
resource "kubernetes_manifest" "dev_team" {
  ...
  manifest = {
    ...
    spec = {
      description = "EKS development team with AWS Bedrock and Claude Code harness"
      agents = [
      ...
      sharedResources = {
        ...
        sandboxClassRef = { name = "eks-sandbox" }
        modelConfigRef  = { name = "claude-bedrock" }
      }
      dispatcherRef = { name = "eks-dev-router" }
      policyRef     = { name = "eks-dev-policy" }
    }
  }
  depends_on = [
      ...
      tasks = [
        {
          ...
          type        = "design"
          description = "Define a simple GET /hello endpoint that returns {\"message\": \"Hello from KARO on EKS!\"}. Create an OpenAPI snippet."
          deps        = []
          priority    = "High"
